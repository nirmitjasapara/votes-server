...
votingRouter.route("/").post(requireAuth, jsonParser, (req, res, next) => {
  ...
  VotingService.vote(req.app.get("db"), newVote)
    .then(vote => {
      res
        .status(201)
...
votingRouter.route("/:_id").get(requireAuth, (req, res, next) => {
  VotingService.getVote(req.app.get("db"), req.user.id, req.params._id)
    .then(vote => {
      res.json(vote);
    })
    .catch(next);
